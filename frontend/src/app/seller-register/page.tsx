...
export default function SellerRegisterPage() {
  ...
  const sendEmailOtp = async () => {
    ...
    try {
      const res = await api.post("/otp/send-email", { email: bizEmail });
      const dev = res.data?.data?.devOtp;
      setDevEmailOtp(dev || null);
      setEmailOtpSent(true);
      setEmailOtp(dev || "");
      startResendTimer();
      setTimeout(() => emailOtpRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }), 100);
      toast.success(dev ? "OTP generated (email not configured — see below)" : `OTP sent to ${bizEmail}. Please check your inbox.`);
    } catch (err: any) {
  ...
  const submitBusiness = () => {
    if (verifyingEmail) {
      toast.error("Please wait, verifying email...");
      return;
    }
    if (!validateBusiness()) {
  ...
  const submitProducts = () => {
    const filledProducts = products.filter((p) => p.name.trim() && p.imageFile);
    if (filledProducts.length < 3) {
      toast.error("Please add at least 3 products with a name and photo");
      return;
    }
    if (!termsAccepted) {
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <form onSubmit={submitAccount} className="p-8 space-y-4">
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-xs text-gray-500">
                  I agree to the{" "}
                  <Link href="/terms" target="_blank" onClick={(e) => e.stopPropagation()} className="text-[#0052cc] font-semibold hover:underline">Terms of Service</Link>
                  {" "}and{" "}
                  <Link href="/privacy" target="_blank" onClick={(e) => e.stopPropagation()} className="text-[#0052cc] font-semibold hover:underline">Privacy Policy</Link>
                </span>
              </label>
                    ...
                    <div ref={emailOtpRef} className="mt-3 space-y-3 bg-blue-50 border border-blue-100 rounded-xl p-4">
                      ...
                      {devEmailOtp && (
                        <p className="text-xs bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-amber-700">
                          Email not configured (dev mode) — Dev OTP: <span className="tracking-widest font-black">{devEmailOtp}</span>
                        </p>
                      )}
                      <div className="flex gap-2">
                        <input
                        ...
                        <button
                          ...
                          className="bg-[#003580] hover:bg-[#002560] disabled:opacity-60 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors"
                        >
            ...
            <div className="p-8">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 flex gap-2 mb-6">
                ...
                <p className="text-xs text-yellow-800">
                  Adding <strong>3 Product Names with Photos</strong> is mandatory. Clear photos help increase buyer enquiries.
                </p>
              </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer mt-5 p-3 rounded-xl hover:bg-gray-50 transition">
                ...
                <span className="text-xs text-gray-600 leading-relaxed">
                  ...
                  with IndiaMart's{" "}
                  <Link href="/terms" target="_blank" className="text-[#003580] font-semibold underline">Terms & Conditions</Link>{" "}
                  and{" "}
              ...
              <div className="flex gap-3 mt-3">
                ...
                <button
                  ...
                  disabled={!termsAccepted || products.filter((p) => p.name.trim() && p.imageFile).length < 3}
              ...
              <p className="text-center text-[11px] text-gray-400 mt-3">
                {products.filter((p) => p.name.trim() && p.imageFile).length}/3 products added (name + photo required)
              </p>
            </div>
          </div>
        )}
