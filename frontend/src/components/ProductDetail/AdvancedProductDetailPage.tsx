...
export default function AdvancedProductDetailPage({
  ...
  // Fetch real reviews for this product — replaces the previous hardcoded demo list
  useEffect(() => {
  ...
  // The seller's uploaded product video lives in the images array as a
  // type:"video" entry — there is no separate product.videoUrl field.
  const uploadedVideoUrl = useMemo(() => {
    return (product.images || []).find((img: any) => img.type === "video")?.url;
  }, [product.images]);
            ...
            <div className="lg:sticky lg:top-24 space-y-3">
              ...
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="px-4 py-2.5 border-b border-gray-100">
                  ...
                  {/* Trust bar — only for sellers actually marked isVerified in the DB */}
                  {product.seller.verified && (
                ...
                <div className="px-4 py-3 space-y-2">
                  ...
                  {/* WhatsApp Button — only shown when the seller opted in a real WhatsApp number */}
                  {(product.seller as any).whatsappNumber && (
                    <button
                      onClick={() => protect(() => {
                        const rawPhone = String((product.seller as any).whatsappNumber).replace(/\D/g, "");
                        const phone = rawPhone.startsWith("91") ? rawPhone : "91" + rawPhone;

                        if (phone.length !== 12) {
                          toast.error("Seller's WhatsApp number appears invalid. Please use Send Inquiry.");
                          return;
                        }
        ...
        <div className="mt-8 border-t border-gray-200 pt-6" id="reviews-section">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200 p-8 mb-8">
            <div className="grid md:grid-cols-2 gap-12">
              ...
              <div className="space-y-3">
                {reviewDistribution.map(({ star, percentage }) => (
                  <div key={star} className="flex items-center gap-3">
                    <span className="text-sm text-gray-600 w-14 shrink-0">
                      {star} star{star !== 1 ? "s" : ""}
                    </span>
          ...
          <div className="space-y-4 mb-8">
            ...
            {reviewsLoading ? (
              ...
              visibleReviews.map((r) => (
                <div key={r._id} className="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          ✓ Verified Purchase • {new Date(r.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                        </p>
                      </div>
                    </div>
        ...
        <div className="mt-12 border-t border-gray-200 pt-8">
          ...
          {uploadedVideoUrl ? (
            <div className="rounded-xl overflow-hidden h-96 bg-gray-900">
              {uploadedVideoUrl.includes('youtube') || uploadedVideoUrl.includes('youtu.be') ? (
                <iframe
                  width="100%"
                  height="100%"
                  src={uploadedVideoUrl.replace('watch?v=', 'embed/').replace('youtu.be/', 'youtube.com/embed/')}
            ...
            <div className="w-full bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl overflow-hidden h-96 flex items-center justify-center">
              <div className="text-center">
                <div className="text-6xl mb-4">▶️</div>
                <p className="text-white text-lg font-semibold">No Demo Video</p>
                <p className="text-gray-400 text-sm mt-2">This seller hasn't uploaded a product video yet</p>
              </div>
            </div>
          )}
        </div>
