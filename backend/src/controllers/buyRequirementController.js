...
export const getBuyRequirements = asyncHandler(async (req, res) => {
  ...
  // Build filters — only public RFQs visible to sellers
  ...
  if (search) filters.productName = new RegExp(search, "i");

  console.log("[RFQ] getBuyRequirements filters:", JSON.stringify(filters));
  ...
  console.log(`[RFQ] Found ${buyRequirements.length} requirements (total: ${totalCount})`);
...
export const getBuyRequirementById = asyncHandler(async (req, res) => {
  ...
  const buyRequirement = await BuyRequirement.findById(requirementId)
    ...
    .populate("category", "name")
    .populate("subCategory", "name")
    .populate("responses.supplier", "name companyName email city state isTrusted businessType yearEstablished averageRating numReviews");
...
export const respondToBuyRequirement = asyncHandler(async (req, res) => {
  ...
  if (!quotedPrice) {
    throw new ApiError(400, "Quoted price is required");
  }
...
export const inviteMoreSellers = asyncHandler(async (req, res) => {
  ...
  const updated = await BuyRequirement.findById(requirementId)
    .populate("invitedSellers", "name companyName email city isTrusted");
