...
// Save products from seller registration flow
// Accept up to 10 product images named image_0 ... image_9
const batchImageFields = Array.from({ length: 10 }, (_, i) => ({ name: `image_${i}`, maxCount: 1 }));

router.post("/register-batch", authMiddleware, roleMiddleware("seller"), (req, res, next) => {
  productUpload.fields(batchImageFields)(req, res, (err) => {
    if (err) return res.status(400).json({ success: false, message: err.message });
    next();
  });
}, async (req, res) => {
  console.log(`[register-batch] seller=${req.user?._id} body keys=${Object.keys(req.body || {})}`);
  try {
    const Product = (await import("../models/Product.js")).default;
    ...
    if (!Array.isArray(products) || products.length === 0) {
      return res.status(400).json(new ApiResponse(400, null, "No products provided"));
    }

    let defaultCategory = await Category.findOne({}).select("_id").lean();
    if (!defaultCategory) {
      const created = await Category.create({ name: "General", slug: "general" });
      defaultCategory = { _id: created._id };
    }
    ...
    const productDocs = products
      ...
      .map((p, idx) => {
        ...
        // Build images array from uploaded file for this index
        const imageFile = req.files?.[`image_${idx}`]?.[0];
        const images = imageFile
          ? [{ url: `/uploads/products/${imageFile.filename}`, type: "image" }]
          : [];
    ...
    if (productDocs.length === 0) {
      return res.status(400).json(new ApiResponse(400, null, "No valid product names"));
    }

    const saved = await Product.insertMany(productDocs, { ordered: true });
    console.log(`[register-batch] saved ${saved.length} products for seller=${req.user._id}`);

    // Mark registration as complete — this is the final step
    const User = (await import("../models/User.js")).default;
    await User.findByIdAndUpdate(req.user._id, { profileCompleted: true });
    console.log(`[register-batch] profileCompleted=true for seller=${req.user._id}`);

    return res.status(201).json(new ApiResponse(201, { count: saved.length }, `${saved.length} products saved`));
  } catch (err) {
    console.error("[register-batch] ERROR:", err.message, err.stack?.split('\n')[1]);
