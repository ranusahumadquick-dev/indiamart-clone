...
export const getProducts = asyncHandler(async (req, res) => {
  ...
  if (search) {
    ...
    query.$or = [{ name: pattern }, { description: pattern }, { sku: pattern }];
  }
...
export const getUsers = asyncHandler(async (req, res) => {
  ...
  // Admin accounts are never shown in this list (buyer/seller management only)
  ...
  // Hide soft-deleted accounts unless explicitly requesting them
  if (filter !== "deleted") query.isDeleted = { $ne: true };
  ...
  const stats = counts[0] || { total: 0, buyers: 0, sellers: 0, active: 0, verified: 0 };
...
// isVerified is the buyer-facing "trusted seller" badge — independent of
// sellerStatus (the pending→under_review→approved/rejected onboarding
// pipeline, which is the only thing that gates live publishing). Admin can
// flip this badge on/off at will, but only ON TOP of an already-approved
// seller — a seller who isn't approved yet has "no sense of isVerified" and
// must never show as verified/trusted to buyers.
export const verifyUser = asyncHandler(async (req, res) => {
  const { isVerified } = req.body;
  if (isVerified === undefined) throw new ApiError(400, "isVerified is required");

  const target = await User.findById(req.params.id).select("role sellerStatus");
  if (!target) throw new ApiError(404, "User not found");

  if (isVerified && target.role === "seller" && target.sellerStatus !== "approved") {
    throw new ApiError(400, "Seller must be approved before they can be marked verified/trusted");
  }
...
// Freeze blocks a seller from publishing products live, without logging
// them out (isActive) or reversing their document verification (isVerified).
// Existing live products stay live — this only blocks future publish actions.
export const freezeSeller = asyncHandler(async (req, res) => {
  const { frozen, reason } = req.body;
  if (frozen === undefined) throw new ApiError(400, "frozen is required");

  const target = await User.findById(req.params.id).select("role");
  if (!target) throw new ApiError(404, "User not found");
  if (target.role !== "seller") throw new ApiError(400, "Only seller accounts can be frozen");

  const update = {
    accountFrozen: frozen,
    accountFrozenReason: frozen ? (reason || "") : "",
    accountFrozenAt: frozen ? new Date() : null,
  };
  ...
  res.json(new ApiResponse(200, { user }, `Seller ${frozen ? "frozen" : "unfrozen"}`));
});

// ─── DELETE /admin/users/:id ─────────────────────────────────────────
// Soft delete — account is deactivated, not erased. A deleted seller's
// products are hidden immediately so nothing keeps showing a broken/
// sellerless listing on the storefront.
export const deleteUser = asyncHandler(async (req, res) => {
  ...
  // Tombstone email/phone so they're released — both fields carry a unique
  // DB index, so a future signup with the same email/phone would otherwise
  // 500 on that index even after this row is marked deleted.
  const user = await User.findByIdAndUpdate(
    req.params.id,
    {
      isDeleted: true,
      deletedAt: new Date(),
      isActive: false,
      email: `deleted_${stamp}_${existing.email}`,
      phone: `deleted_${stamp}_${existing.phone}`,
    },
    { new: true }
