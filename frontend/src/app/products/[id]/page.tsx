  ...
  if (product.variants && product.variants.length > 0 && product.variantTypes && product.variantTypes.length > 0) {
    const advancedProduct = {
      ...
      seller: {
        ...
        // Only a real, seller-opted-in WhatsApp number — never fall back
        // to the account phone, that's a different, non-consented number.
        whatsappNumber: product.seller?.whatsapp?.displayOnProfile && product.seller?.whatsapp?.number
          ? product.seller.whatsapp.number
          : "",
      },
            ...
            <div className="lg:sticky lg:top-6 space-y-3">
              ...
              <form id="inquiry" onSubmit={sendInquiry} className="bg-white rounded-lg border border-gray-200 p-3 space-y-2">
                ...
                <button
                  ...
                  className="w-full bg-orange-600 hover:bg-orange-700 disabled:bg-orange-400 disabled:cursor-not-allowed text-white font-semibold py-1.5 rounded-lg transition text-xs flex items-center justify-center gap-1.5"
                >
                  {sendingInquiry ? (
                    <>
