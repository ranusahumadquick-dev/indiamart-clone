...
export const createService = async (req, res, next) => {
  try {
    ...
    // ── Normalize arrays: FormData sends 1 item as string, multiple as array ──
    const toArray = (val) => {
      if (!val) return [];
    ...
    // ── Explicit field-level validation with specific messages ──
    const missing = [];
    ...
    if (description.trim().length < 20) {
      return next(new ApiError(400, 'Description must be at least 20 characters'));
    }
    ...
    // ── Handle uploaded images (req.files is an array when using .array()) ──
    const backendUrl = process.env.BACKEND_URL || 'http://localhost:8000';
    let images = [];

    if (Array.isArray(req.files) && req.files.length > 0) {
      // serviceUpload.array('images', 5) → req.files is a plain array
      images = req.files.map((file) => ({
        url: `${backendUrl}/uploads/services/${file.filename}`,
        alt: serviceName.trim(),
      }));
    } else if (req.files && req.files.images) {
      // fields() format fallback
      const imgArr = Array.isArray(req.files.images) ? req.files.images : [req.files.images];
      images = imgArr.map((file) => ({
        url: `${backendUrl}/uploads/services/${file.filename}`,
        alt: serviceName.trim(),
      }));
    }
