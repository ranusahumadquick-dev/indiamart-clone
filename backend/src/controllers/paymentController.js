...
/**
 * Verify a Razorpay callback and load the matching pending payment.
 * Rejects if signature is invalid, the signed order is not bound to this
 * payment record, the payment is not owned by the caller, or it is not pending.
 * Prevents replaying a valid (order|payment) signature against a different
 * or higher-value payment record.
 */
const verifyAndLoadPayment = async ({
  ...
  if (!razorpayOrderId || !razorpayPaymentId || !razorpaySignature || !paymentDbId) {
    throw new ApiError(400, "razorpayOrderId, razorpayPaymentId, razorpaySignature and paymentId are required");
  }
  ...
  // Bind the signed order to this exact payment record
...
const createRazorpayOrder = asyncHandler(async (req, res) => {
  const { amount: clientAmount, paymentFor = "product", metadata = {} } = req.body;

  // Never trust a client-supplied amount for plan purchases — resolve the
  // authoritative price from the plan on the server.
  let amount = clientAmount;
  if (paymentFor === "subscription" || paymentFor === "buyer_subscription") {
    if (!metadata.planId) throw new ApiError(400, "planId is required for subscription payments");
...
const verifyPayment = asyncHandler(async (req, res) => {
  ...
  const payment = await verifyAndLoadPayment({
    razorpayOrderId,
    razorpayPaymentId,
    razorpaySignature,
    paymentDbId,
...
const verifyBuyerPayment = asyncHandler(async (req, res) => {
  ...
  // Bind plan to what was actually ordered/paid for
  if (payment.metadata?.planId?.toString() !== plan._id.toString()) {
    throw new ApiError(400, "Plan does not match the paid order");
  }
