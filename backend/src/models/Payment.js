...
const paymentSchema = new mongoose.Schema({
  ...
  paymentFor: {
    type: String,
    enum: ['subscription', 'buyer_subscription', 'product', 'listing', 'advertisement', 'other'],
    required: true
  },
  status: {
    type: String,
...
paymentSchema.methods.updateStatus = async function(status, additionalData = {}) {
  ...
  switch (status) {
    case 'completed':
      ...
      if (additionalData.razorpayPaymentId) this.razorpayPaymentId = additionalData.razorpayPaymentId;
      if (additionalData.razorpaySignature) this.razorpaySignature = additionalData.razorpaySignature;
      break;
    case 'failed':
...
paymentSchema.methods.markCompleted = async function(razorpayPaymentId, razorpaySignature) {
  return this.updateStatus('completed', { razorpayPaymentId, razorpaySignature });
};
