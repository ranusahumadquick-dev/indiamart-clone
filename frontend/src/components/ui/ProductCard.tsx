...
import { HiMiniShieldCheck } from "@/lib/icons";
...
const ProductCard = ({
  ...
  return (
    <>
      <div className="group relative bg-white rounded-2xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)] hover:shadow-[0_20px_40px_-12px_rgba(15,23,42,0.18)] hover:-translate-y-1 hover:border-blue-100 transition-all duration-300 flex flex-col overflow-hidden">

        {/* ── Image ── */}
        <Link href={`/products/${_id}`} className="block relative overflow-hidden bg-gray-50" style={{ aspectRatio: "4/3" }}>
          ...
          {/* Stock badge */}
          {isOutOfStock ? (
            <span className="absolute top-3 left-3 bg-gray-900/90 text-white text-[10px] font-bold tracking-wide px-2.5 py-1 rounded-lg shadow-sm backdrop-blur-sm">
              OUT OF STOCK
            </span>
          ) : discount > 0 ? (
            <span className="absolute top-3 left-3 bg-gradient-to-r from-rose-500 to-red-500 text-white text-[10px] font-bold tracking-wide px-2.5 py-1 rounded-lg shadow-md shadow-red-500/30">
              {discount}% OFF
            </span>
          ) : badge ? (
            <span className={`absolute top-3 left-3 text-white text-[10px] font-bold tracking-wide px-2.5 py-1 rounded-lg shadow-md
              ${{ new: "bg-gradient-to-r from-emerald-500 to-green-500 shadow-green-500/30", bestseller: "bg-gradient-to-r from-purple-500 to-violet-500 shadow-purple-500/30", hot: "bg-gradient-to-r from-orange-500 to-amber-500 shadow-orange-500/30", limited: "bg-gradient-to-r from-red-600 to-rose-600 shadow-red-500/30" }[badge]}`}>
              {{ new: "NEW", bestseller: "BESTSELLER", hot: "HOT DEAL", limited: "LIMITED" }[badge]}
            </span>
          ) : stock > 0 && stock <= 10 ? (
            <span className="absolute top-3 left-3 bg-gradient-to-r from-amber-500 to-yellow-500 text-white text-[10px] font-bold tracking-wide px-2.5 py-1 rounded-lg shadow-md shadow-amber-500/30">
              Only {stock} left
            </span>
          ) : null}
          ...
          {isVerified && (
            <button
              ...
              className="absolute top-3 right-3 bg-white/95 backdrop-blur-sm text-blue-700 text-[9px] font-bold px-2 py-1 rounded-full flex items-center gap-1 shadow-md hover:bg-blue-600 hover:text-white transition-colors"
            >
              <HiMiniShieldCheck className="w-3.5 h-3.5" /> TrustSEAL
            </button>
          )}
          ...
          <div className="absolute bottom-3 right-3 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-1 group-hover:translate-y-0">
            <div className="bg-white/95 backdrop-blur-sm rounded-full p-1.5 shadow-md hover:bg-blue-50 transition-colors">
              <WishlistButton productId={_id} iconSize="w-3.5 h-3.5" />
            </div>
            <button
              onClick={handleCompareToggle}
              title={inCompare ? "Remove from compare" : "Compare"}
              className={`bg-white/95 backdrop-blur-sm rounded-full p-1.5 shadow-md transition-colors ${inCompare ? "text-blue-600 bg-blue-50" : "text-gray-400 hover:text-blue-500 hover:bg-blue-50"}`}
            >
            ...
            <button
              ...
              className={`bg-white/95 backdrop-blur-sm rounded-full p-1.5 shadow-md transition-colors ${inBulk ? "text-orange-500 bg-orange-50" : "text-gray-400 hover:text-orange-500 hover:bg-orange-50"}`}
            >
        ...
        <div className="flex flex-col flex-1 px-3.5 pt-3 pb-3.5 gap-1.5">
          ...
          <div className="flex items-center gap-1.5 h-5">
            <span className="w-4 h-4 rounded-full bg-blue-50 flex items-center justify-center shrink-0">
              {isVerified && <HiOutlineShieldCheck className="w-2.5 h-2.5 text-blue-600" />}
            </span>
            <p className="text-[11px] font-semibold truncate text-blue-700">
              {companyName || <span className="text-gray-300 font-normal">—</span>}
            </p>
          </div>

          {/* Product name — always 2 lines */}
          <Link href={`/products/${_id}`}>
            <h3 className="text-[13.5px] font-bold text-gray-900 line-clamp-2 leading-[1.35] h-[2.7em] group-hover:text-blue-700 transition-colors">
              {name}
            </h3>
          </Link>
          ...
          <div className="h-5">
            {brand ? (
              <span className="inline-flex items-center text-[10px] text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full font-semibold">
                {brand.brandName}
              </span>
            ) : category ? (
              <span className="inline-flex items-center text-[10px] text-gray-500 bg-gray-50 border border-gray-200 px-2 py-0.5 rounded-full font-medium">
                {category}
              </span>
            ) : null}
          </div>
          ...
          {/* Price */}
          <div className="flex items-baseline gap-1.5 flex-wrap bg-gradient-to-r from-blue-50/70 to-transparent -mx-1 px-1 py-1 rounded-lg">
            <span className="text-[19px] font-extrabold text-gray-900 leading-none tracking-tight">{formatINR(price)}</span>
            {comparePrice && comparePrice > price && (
              <span className="text-xs text-gray-400 line-through leading-none">{formatINR(comparePrice)}</span>
            )}
            <span className="text-[11px] text-gray-400 leading-none font-medium">/ {priceUnit}</span>
          </div>
          ...
          <div className="flex items-center justify-between gap-2 h-5">
            ...
            {averageRating > 0 ? (
              <div className="flex items-center gap-1 shrink-0">
                ...
                <span className="text-[11px] font-bold text-amber-600">{averageRating.toFixed(1)}</span>
                {numReviews > 0 && <span className="text-[10px] text-gray-400">({numReviews})</span>}
              </div>
            ) : (
              <span className="text-[10px] text-gray-300 italic">No ratings</span>
            )}
          </div>
          ...
          {/* Location */}
          <div className="flex items-center gap-1.5 text-[11px] text-gray-500 border-t border-gray-100 pt-2.5 h-8">
            <HiOutlineMapPin className="w-3.5 h-3.5 shrink-0 text-gray-400" />
            {location ? <span className="truncate font-medium">{location}</span> : <span className="text-gray-300">Location not set</span>}
          </div>
          ...
          <div className="flex gap-2 mt-0.5">
            <button
              ...
              className="flex-1 text-center whitespace-nowrap text-[11.5px] font-bold text-blue-600 border-2 border-blue-100 bg-blue-50/50 rounded-xl px-2 py-2.5 hover:bg-blue-600 hover:border-blue-600 hover:text-white hover:shadow-lg hover:shadow-blue-500/25 transition-all duration-200"
            >
              View Details
            </button>
            <button
              onClick={() => protect(() => router.push(`/products/${_id}#inquiry`))}
              className="flex-1 flex items-center justify-center gap-1.5 whitespace-nowrap text-[11.5px] font-bold text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 hover:shadow-lg hover:shadow-orange-500/30 active:scale-95 rounded-xl px-2 py-2.5 transition-all duration-200"
            >
