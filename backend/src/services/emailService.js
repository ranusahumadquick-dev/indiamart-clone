...
export async function sendPasswordResetEmail(email, name, resetLink) {
  ...
  if (!t) {
    ...
    console.log("  ⚠️  Set GMAIL_USER + GMAIL_APP_PASSWORD in .env to send real emails");
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    return { sent: false, dev: true };
  }
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:40px 40px 32px;">
            ...
            <p style="color:#64748b;font-size:14px;line-height:1.6;margin:0 0 6px;">Hi <strong>${name || "there"}</strong>,</p>
            <p style="color:#64748b;font-size:14px;line-height:1.6;margin:0 0 28px;">We received a request to reset your IndiaMart password. Click the button below — this link is valid for <strong>15 minutes</strong>.</p>
            <div style="text-align:center;margin:0 0 28px;">
              <a href="${resetLink}" style="display:inline-block;background:#0052cc;color:#fff;font-size:16px;font-weight:700;padding:14px 36px;border-radius:8px;text-decoration:none;">Reset My Password</a>
            </div>
            <p style="color:#64748b;font-size:13px;margin:0 0 8px;">Or copy this link:</p>
            <div style="background:#f0f4ff;border-radius:8px;padding:12px 16px;word-break:break-all;margin:0 0 24px;">
              <a href="${resetLink}" style="color:#0052cc;font-size:12px;text-decoration:none;">${resetLink}</a>
            </div>
            <div style="background:#fff8ed;border-left:4px solid #f59e0b;border-radius:4px;padding:12px 16px;">
              <p style="color:#92400e;font-size:13px;margin:0;">🔒 If you didn't request this, please ignore this email. Your account remains secure.</p>
            </div>
          </td>
        </tr>
        <tr>
          <td style="background:#f8fafc;padding:20px 40px;border-top:1px solid #e2e8f0;text-align:center;">
            <p style="color:#94a3b8;font-size:11px;margin:0;">© ${new Date().getFullYear()} IndiaMart B2B Marketplace</p>
          </td>
        </tr>
      </table>
    </td></tr>
