...
function VerificationStatusCard({ user, onRefresh }: { user: VerificationUser | null; onRefresh: () => Promise<void> }) {
  ...
  const canRequest = !user.isVerified && !user.verificationRequested && completedChecks >= 2;
  ...
  return (
    <div className={`rounded-xl border p-5 shadow-sm ${user.isVerified ? "bg-blue-50 border-blue-200" : user.verificationRequested ? "bg-amber-50 border-amber-200" : "bg-white border-gray-100"}`}>
      {/* Header */}
        ...
        <div className="space-y-3">
          ...
          {user.verificationRequested && (
            <div className="bg-amber-100 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
              ...
              <p className="text-xs">
                Submitted {user.verificationRequestedAt
                  ? new Date(user.verificationRequestedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })
                  : "recently"}.
