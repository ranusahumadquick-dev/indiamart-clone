...
export const adminListSellers = asyncHandler(async (req, res) => {
  ...
  const [sellers, total] = await Promise.all([
    User.find(filter)
      .select("name email phone companyName gstNumber gstRegDate gstAge gstVerified sellerStatus sellerStatusNote sellerDocs isVerified createdAt updatedAt city state pincode annualTurnover")
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(Number(limit))
      .lean(),
...
// Approval only unlocks live publishing (sellerStatus). It does NOT grant the
// buyer-facing "verified/trusted" badge (isVerified) — that's a separate,
// independent toggle admin sets afterward via /admin/users/:id/verify.
// ═══════════════════════════════════════════════════════════════════════════════
...
export const adminRejectSeller = asyncHandler(async (req, res) => {
  ...
  await User.findByIdAndUpdate(sellerId, {
    ...
    sellerStatusUpdatedAt: new Date(),
    isVerified:            false, // can't hold the trusted badge without being approved
  });
