...
const notifIcons: Record<string, typeof HiOutlineBell> = {
  inquiry_reply: HiOutlineChatBubbleLeftRight,
  sample_status: HiOutlineArchiveBox,
  requirement_response: HiOutlineClipboardDocumentList,
  product_approved: HiOutlineCheckCircle,
  product_rejected: HiOutlineXCircle,
  order_update: HiOutlineShoppingCart,
  general: HiOutlineBell,
};

function RoleBadgeLabel({ role }: { role?: string }) {
  if (role === "seller") return <span className="inline-flex items-center gap-1"><HiOutlineTag className="w-3 h-3" />Seller</span>;
  if (role === "admin") return <span className="inline-flex items-center gap-1"><Crown className="w-3 h-3" />Admin</span>;
  return <span className="inline-flex items-center gap-1"><HiOutlineShoppingCart className="w-3 h-3" />Buyer</span>;
}
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      ...
      setShowSuggestions(true); // open dropdown immediately so the loading spinner is visible
      try {
  ...
  const POPULAR_SEARCHES = [
    "Steel Pipes", "LED Lights", "Packaging Machines", "Cotton Fabric",
    "Solar Panels", "Hydraulic Cylinders", "Industrial Pumps",
  ];
    ...
    <header className="sticky top-0 z-50 bg-white shadow-md">
      ...
      <div className="bg-[var(--primary-dark)] text-white text-xs py-1.5">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-4 items-center">
            <Link href="/seller-register" className="hover:text-yellow-300 transition font-semibold inline-flex items-center gap-1">
      ...
      <div className="max-w-7xl mx-auto px-4 py-2.5">
        <div className="flex items-center gap-3">
          ...
          {/* Dim backdrop — shown while the search overlay is active, everything
              except the (higher z-index) navbar/search bar dims behind it */}
          {searchActive && (
            <div
              className="fixed inset-0 bg-black/40 backdrop-blur-[1px] z-40"
              onClick={() => { setSearchActive(false); setShowSuggestions(false); }}
            />
          )}

          {/* Search Bar with Autocomplete */}
          <div ref={searchRef} className={`flex-1 max-w-2xl relative transition-all duration-200 ${searchActive ? "z-50 max-w-3xl" : ""}`}>
            <form onSubmit={handleSearch}>
              <div className={`relative rounded-xl transition-all duration-200 ${searchActive ? "shadow-lg ring-2 ring-blue-100" : ""}`}>
                <HiMagnifyingGlass className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${searchActive ? "text-[var(--primary)]" : "text-gray-400"}`} />
                <input
                  type="text"
                  placeholder="Search products, suppliers, categories..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onFocus={() => { setSearchActive(true); if (suggestions.length > 0) setShowSuggestions(true); }}
                  className={`w-full pl-10 pr-24 py-2.5 rounded-xl border text-gray-800 text-sm outline-none transition-all duration-200 ${searchActive ? "border-[var(--primary)] bg-white" : "border-gray-300 bg-gray-50 hover:border-gray-400"}`}
                />
                {searchQuery && (
                  <button
                    type="button"
                    onClick={() => { setSearchQuery(""); setSuggestions([]); setSellerSuggestions([]); setShowSuggestions(false); }}
                    className="absolute right-[92px] sm:right-[104px] top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1"
                ...
                <button
                  type="submit"
                  className="absolute right-1 top-1/2 -translate-y-1/2 bg-[var(--primary)] text-white px-4 py-1.5 rounded-lg hover:bg-[var(--primary-dark)] transition text-sm font-medium flex items-center gap-1.5"
                >
            ...
            {/* Empty-state panel — shown as soon as the search is focused,
                before the user has typed anything to trigger real suggestions */}
            {searchActive && !showSuggestions && !searchQuery.trim() && (
              <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-200 rounded-xl shadow-2xl overflow-hidden">
                <div className="px-4 py-3 text-[11px] text-gray-400 font-semibold uppercase tracking-wide border-b border-gray-50">
                  Popular Searches
                </div>
                <div className="flex flex-wrap gap-2 p-4">
                  {POPULAR_SEARCHES.map((term) => (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-gray-200 text-xs font-medium text-gray-600 hover:border-[var(--primary)] hover:text-[var(--primary)] hover:bg-blue-50 transition"
                    >
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                {seller.isVerified && (
                                  <span className="inline-flex items-center gap-0.5 text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-medium shrink-0">
                          ...
                          notifications.map((notif) => (
                            ...
                            >
                              <span className="shrink-0 mt-0.5 text-gray-500">
                                {(() => {
                                  const NotifIcon = notifIcons[notif.type] || HiOutlineBell;
                                  return <NotifIcon className="w-5 h-5" />;
                                })()}
                              </span>
                              <div className="flex-1 min-w-0">
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full mt-1 w-64 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
                      ...
                      <div className="py-1">
                        ...
                        <Link href={user.role === "admin" ? "/admin/settings" : user.role === "seller" ? "/profile" : "/buyer/settings"} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition" onClick={() => setUserMenuOpen(false)}>
          ...
          {isAuthenticated && user ? (
            <>
              ...
              <div className="px-3 pb-3 border-t border-gray-100">
                <Link href={user?.role === "admin" ? "/admin/settings" : user?.role === "seller" ? "/profile" : "/buyer/settings"}
                  onClick={() => setMobileMenuOpen(false)}
