...
function ProductsContent() {
  ...
  const applyAvailability = (value: "" | "verified" | "samples") => {
    const newFilters = {
      ...filters,
      isVerified: value === "verified" ? "true" : "",
      allowSamples: value === "samples" ? "true" : "",
    };
    setFilters(newFilters);
    syncToURL(newFilters);
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 py-6">
        {/* ── Sort / View Bar — search moved to global navbar, no duplicate here ── */}
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6 shadow-sm">
          {/* Row 1: quick filter dropdowns (left) + sort/view toggle (right) — desktop only */}
          <div className="hidden md:flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3 flex-wrap">
              <div className="relative w-[200px] shrink-0">
                ...
                <input
                  ...
                  className="w-full h-11 border border-gray-200 rounded-lg pl-9 pr-3 text-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 focus:outline-none"
                />
              </div>
              ...
              <select
                ...
                className="h-11 w-[180px] shrink-0 border border-gray-200 rounded-lg px-3 text-sm text-gray-700 bg-white focus:ring-2 focus:ring-blue-200 focus:outline-none"
              >
              ...
              <select
                value={PRICE_PRESETS.some((p) => p.min === filters.minPrice && p.max === filters.maxPrice) ? `${filters.minPrice}-${filters.maxPrice}` : ""}
                onChange={(e) => {
                  if (!e.target.value) return applyPricePreset({ label: "", min: "", max: "" });
                  const preset = PRICE_PRESETS.find((p) => `${p.min}-${p.max}` === e.target.value);
                  if (preset) applyPricePreset(preset);
                }}
                className="h-11 w-[160px] shrink-0 border border-gray-200 rounded-lg px-3 text-sm text-gray-700 bg-white focus:ring-2 focus:ring-blue-200 focus:outline-none"
              >
              ...
              <select
                value={filters.isVerified === "true" ? "verified" : filters.allowSamples === "true" ? "samples" : ""}
                onChange={(e) => applyAvailability(e.target.value as "" | "verified" | "samples")}
                className="h-11 w-[180px] shrink-0 border border-gray-200 rounded-lg px-3 text-sm text-gray-700 bg-white focus:ring-2 focus:ring-blue-200 focus:outline-none"
              >
            ...
            <div className="flex items-center gap-3 shrink-0">
              <select
                ...
                className="h-11 border border-gray-200 rounded-lg px-4 text-sm text-gray-700 bg-white focus:ring-2 focus:ring-blue-200 focus:outline-none"
              >
              ...
              <div className="h-11 flex items-center border border-gray-200 rounded-lg overflow-hidden">
                <button
                  onClick={() => setViewMode("grid")}
                  className={`h-full px-3 flex items-center transition ${viewMode === "grid" ? "bg-blue-50 text-[var(--primary)]" : "text-gray-400 hover:bg-gray-50"}`}
                  title="Grid view"
                >
                ...
                <button
                  ...
                  className={`h-full px-3 flex items-center transition border-l border-gray-200 ${viewMode === "list" ? "bg-blue-50 text-[var(--primary)]" : "text-gray-400 hover:bg-gray-50"}`}
                  title="List view"
                >
          ...
          {/* Row 2: result count (left) + mobile filter toggle (right, mobile only) */}
          <div className="flex items-center justify-between gap-3 mt-3 md:mt-3">
            <p className="text-sm text-gray-500">
              Showing <span className="font-semibold text-gray-800">{filtered.length.toLocaleString("en-IN")}</span>{" "}
              product{filtered.length !== 1 ? "s" : ""}
              {filters.location ? (
                <> in <span className="font-semibold text-gray-800">{filters.location}</span></>
              ) : (
                " across India"
              )}
            </p>
