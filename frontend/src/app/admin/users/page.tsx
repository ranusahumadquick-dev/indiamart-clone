...
const UserManagementPage = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'buyers' | 'sellers' | 'admins' | 'active' | 'verified' | 'pending_verification'>('all');
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`/admin/users?filter=${filter}`);
      if (res.data?.success) {
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
          <div className="flex flex-wrap gap-2">
            {[
              ...
              { value: 'admins', label: 'Admins', count: null },
                  ...
                  <tr key={user._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex flex-wrap gap-2">
                        ...
                        >
                          {user.isVerified ? "Unverify" : user.verificationRequested ? "✓ Approve" : "Verify"}
                        </button>
                        <button
