...
export const getSellers = asyncHandler(async (req, res) => {
  ...
  const [sellers, total] = await Promise.all([
    User.find(filter)
      .select("name companyName avatar businessType businessDescription city state isVerified isEmailVerified isMobileVerified gstNumber yearEstablished avgResponseTime replyCount")
...
function computeProfileScore(user) {
  ...
  if (anyLink) score += 4;
  if (user.companyVideo || user.companyVideos?.length > 0 || user.uploadedVideos?.length > 0) score += 6;
...
export const updateMySellerProfile = asyncHandler(async (req, res) => {
  const ALLOWED = [
    ...
    "companyVideo", "companyVideos", "uploadedVideos", "virtualTourUrl",
  ];
...
export const getSellerProfile = asyncHandler(async (req, res) => {
  ...
  const products = await Product.find({
    ...
    .select("name slug price comparePrice images category subCategory brand averageRating stock minOrderQuantity city state")
...
export const updateWhatsappNumber = asyncHandler(async (req, res) => {
  ...
  // WhatsApp numbers must be unique — no two sellers can share one
  const existing = await User.findOne({
    "whatsapp.number": cleaned,
    _id: { $ne: req.user._id },
  });
  if (existing) {
    throw new ApiError(409, "This WhatsApp number is already in use by another seller.");
  }
