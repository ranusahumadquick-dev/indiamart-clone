...
async function fetchFromMastersIndia(gstin) {
  ...
  try {
    ...
    // Extract principal address from GST response
    const pradr = data.pradr?.addr || data.pradr || {};
    const addrParts = [pradr.bnm, pradr.bno, pradr.flno, pradr.st, pradr.loc]
      .filter(Boolean).join(", ");
    const city    = pradr.dst || pradr.loc || "";
    const pincode = String(pradr.pncd || "").replace(/\D/g, "");

    // PAN is embedded in GSTIN: chars 3–12 (0-indexed: 2 to 11)
    const pan = gstin.length >= 12 ? gstin.substring(2, 12) : "";

    return {
      success:          true,
      businessName:     data.lgnm     || data.tradeNam || data.trade_name || "",
