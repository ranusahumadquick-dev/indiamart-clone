...
const createInquiry = asyncHandler(async (req, res) => {
  ...
  const { message, quantityRequired, preferredDeliveryLocation, subject, sellerId: sellerIdFromBody } =
    req.body;
  ...
  // Check seller ID — use body fallback if product.seller is missing
  const sellerId = product.seller?._id || sellerIdFromBody;
  if (!sellerId) {
    console.error("❌ [Inquiry] Product seller missing and no sellerId in body:", {
      hasSeller: !!product.seller,
      sellerIdFromBody,
    });
    throw new ApiError(400, "Seller information not found. Please try again.");
  }
